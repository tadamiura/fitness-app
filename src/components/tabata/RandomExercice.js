...
const RandomExercice = (props) => {

    const [randomExercice, setRandomExercice] = useState(null)

    const getRandomExercice = () => {
        // const url = 'http://localhost:8080/api/exercice'
        axios.get(`http://localhost:8080/api/exercice`)
        .then((res) => setRandomExercice(res.data))
    }

    useEffect(() => getRandomExercice(), [])

    return randomExercice === null ?
    <p>Loading ...</p>
    :(
        <div className='random-exercice'>
            {randomExercice.map( exercice => 
                <div key={exercice.id}>
                    {exercice.name}
                </div>)}
        </div>
